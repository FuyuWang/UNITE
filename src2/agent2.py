...
class Agent(object):
    def __init__(self, model_defs, num_pe=4096, bandwidth=4096, num_pipe=8, feat_size=128, num_episodes=32, lr=1e-3):
        ...
        self.used_bandwidth = np.zeros(self.num_episodes)
    ...
    def step(self, state, instruction):
        ...
        if 1 <= instruction <= self.num_pipe:
            ...
            mask1 = torch.from_numpy(np.expand_dims(self.pe_space, 0) > np.expand_dims(remained_pes, 1)).to(self.device).float() * -1000000.

            remained_bw = self.bandwidth - self.used_bandwidth - self.bw_space[0] * (self.num_pipe - instruction)
            mask2 = torch.from_numpy(np.expand_dims(self.bw_space, 0) > np.expand_dims(remained_bw, 1)).to(
                self.device).float() * -1000000.

            action, log_prob, log_prob_mask, entropy, value = self.a2c(state, instruction, [mask1, mask2])
    ...
    def record_resource(self, action):
        self.used_pes += action[:, 0]
        self.used_bandwidth += action[:, 1]
    ...
    def compute_policy_loss(self, filter=False):
        ...
        policy_loss = dis_rewards * (-1 * self.log_probs * self.log_prob_masks).sum(dim=0).sum(dim=-1)
...
class A2CNet(nn.Module):
    ...
    def forward(self, state, instruction, mask):
        ...
        if 1 <= instruction <= num_pipe:
            ...
            self.lstm_value = (h, x)
            # h = feat
            num_pe_score = self.num_pe_decoder(h) + mask[0]
            ...
            action1 = num_pe_action
            log_prob1 = num_pe_log_prob
            log_prob_mask1 = ((mask[0] == 0).sum(dim=-1) > 1).float()

            band_score = self.band_decoder(h) + mask[1]
            ...
            action2 = band_action
            log_prob2 = band_log_prob
            log_prob_mask2 = ((mask[1] == 0).sum(dim=-1) > 1).float()

            action = torch.stack([action1, action2], dim=1)
            log_prob = torch.stack([log_prob1, log_prob2], dim=1)
            log_prob_mask = torch.stack([log_prob_mask1, log_prob_mask2], dim=1)
            entropy = num_pe_density.entropy() + band_density.entropy()
        else:
            ...
            action = layer_pipe_action
            log_prob = torch.stack([layer_pipe_log_prob, layer_pipe_log_prob.new_zeros(self.num_episodes)], dim=1)
            entropy = layer_pipe_density.entropy()
            log_prob_mask = torch.stack([((mask == 0).sum(dim=-1) > 1).float(), layer_pipe_log_prob.new_zeros(self.num_episodes)], dim=1)
