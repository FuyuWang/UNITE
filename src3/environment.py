...
class MaestroEnvironment(object):
    def __init__(self, model_defs, input_size=32, dataflow="dla",
                 ...
                 num_pipe=8, pe_alloc=None, fitness='latency', num_episodes=None):
        ...
        self.pe_alloc = pe_alloc
        self.total_steps = 1+num_pipe+model_defs.shape[0]
        self.fitness = fitness
        self.num_episodes = num_episodes
        self.mode = num_pipe + 1
        self.solutions = np.zeros((num_episodes, self.total_steps), dtype=np.int32)
        self.solutions[:, 0] = num_pipe
        self.solutions[:, 1:num_pipe+1] = pe_alloc
        self.sol_record = []
        self.best_sol = None
    ...
    def reset(self):
        ...
        self.solutions[:, 1:self.num_pipe+1] = self.pe_alloc
        ...
        if self.model_defs_saved[self.mode - self.num_pipe - 1][3] > \
                self.model_defs_saved[self.mode - self.num_pipe - 1][0] and \
    ...
    def step(self, action):
        self.solutions[:,self.mode] = action

        self.mode += 1

        if self.num_pipe < self.mode < self.total_steps:
            state = np.zeros((self.num_episodes, self.num_pipe+self.model_defs.shape[1]))
            state[:, 0:self.num_pipe] = self.solutions[:, 1:self.num_pipe + 1] / self.constraint_value[0]
        ...
        # print(state.shape)
        return self.solutions, state, reward, reward_saved
