...
def rl_search(model_defs):
    ...
    #     print(num_pipe)
    print(model_defs)
    agent = Agent(model_defs=model_defs, num_pe=opt.num_pe, max_pipe=max_pipe,
    ...
    env = MaestroEnvironment(model_defs=model_defs, input_size=opt.input_size, dataflow="dla",
                             ...
                             max_pipe=max_pipe, fitness='latency', num_episodes=num_episodes)
    for epoch in range(pipe_epochs):
        ...
        for t in range(0, env.total_steps):
            action, log_prob, log_prob_mask, entropy, value = agent.step(state, t)
            solutions, state, reward, reward_saved, done = env.step(action)
...
if __name__ == "__main__":
    ...
    #     # for model in ['resnet18', 'UNet', 'mobilenet_v2', 'vgg16', 'mnasnet']:
